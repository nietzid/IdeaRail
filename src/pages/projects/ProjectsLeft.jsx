...
export default function ProjectsLeft({currentProject, setCurrentProject}) {
    ...
    async function getProjects() {
        await supabase.from('projects').select('*').eq('group_id', user.user_metadata.group_id).then((res) => {
            if (res.error) 
            console.log(error);
            else 
    ...
    const subscribe = () => supabase.channel('projects-channel').on('postgres_changes', {
        event: '*',
        schema: 'public',
        table: 'projects', 
        ...
        <>
            <div className='p-1'>
                <div className='flex justify-between my-4'>
                    ...
                    <button type="button"
                        onClick={openModal}
                        className="hidden md:inline-flex my-auto justify-end items-center rounded-md border border-transparent bg-indigo-600 px-6 py-3 text-base font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
            ...
            <ProjectsFormCard isOpen={isOpen}
                setIsOpen={setIsOpen}/>
        </>
    )
}
