...
export default function SprintForm({isOpen, setIsOpen, category, projectId, sprintId}) {
    ...
    async function handleSubmit(e) {
        ...
        try {
            setIsLoading(true);
            const {data, error} = await supabase.from('sprint').insert([{
                ...
                setIsLoading(false);
                setNotificationMessage(`${category} berhasil dibuat!`)
                            ...
                            <Transition.Child as={Fragment}
                                ...
                                <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
                                    <div className="px-6 py-6 lg:px-8">
                                        <h3 className="mb-4 text-xl font-medium text-gray-900 dark:text-white">Masukan {category} baru</h3>
                                        <form className="space-y-6"
                                            onSubmit={handleSubmit}>
                                            <div>
                                                <label htmlFor="content" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Isi {category}</label>
                                                <textarea name="content" id="content" cols="30" rows="10" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder='Deskripsi singkat project yang akan dikerjakan' required></textarea>
                                            </div>
                                            <button type="submit"
                                                className={
