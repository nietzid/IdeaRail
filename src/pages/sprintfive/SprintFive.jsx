...
export default function SprintFive(props) {
  ...
  async function finalizeSprint() {
    ...
    await supabase
      .from("projects")
      .update({ progress: Math.ceil((5 / 6) * 100) })
  ...
  return (
    <>
      <header className="bg-white mx-4 md:mx-16 mt-4">
        <div className="mx-auto max-w-screen py-6">
          ...
          <p className="text-lg py-2 tracking-tight text-gray-900">
            Pada Sprint ini kamu harus menentukan How Might We (HMW) berdasarkan
            pain & gain yang sebelumnya kamu temukan. Harap diingat kembali
            bahwa hmw harus bertujuan untuk menyelesaikan challenge yang kamu
            tentukan diawal. Kalimat challenge kamu adalah <br />
            <span className="font-bold">"{challenge}"</span>.
          </p>
          <button
            type="button"
            onClick={() => setShowPainAndGain(true)}
      ...
      />
      <PainAndGain
        isOpen={showPainAndGain}
        setIsOpen={setShowPainAndGain}
        projectId={
          typeof projectId === "object" ? projectId.projectId : projectId
        }
      />
